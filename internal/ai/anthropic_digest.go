...
func digestPrompt(data ResultsDigestData, cfg CommentConfig) string {
	...
	b.WriteString("2. THE FINAL RESULT IS THE TRUTH. Who got the exact score, who called the result, who whiffed, and how many points each scored are decided ONLY by the top-level \"score\" (the settled regulation result) and each pick's \"points\" in \"picks\" — those are FINAL. \"live_story\" and \"live_snapshots\" are the IN-PLAY JOURNEY: PROVISIONAL, mid-match, often out of date. A player can be sitting on the exact score and then be OVERTAKEN by a late goal (a 3-1 that becomes 4-1 in stoppage). CRUCIAL: the game often ENDS right after that last goal, so the live commentary may NEVER have narrated it — \"live_story\" can STOP on a now-wrong scoreline and its last line is NOT the final state. NEVER say a player \"nailed it\" / \"got the exact score\" / \"called it\" based on a live line or a snapshot frame — judge exact/correct/wrong SOLELY from the final \"points\" in \"picks\" (e.g. an exact hit is the top points tier; 0 means wrong result). If a live line says someone was exact but the final \"score\"/\"points\" disagree, the FINAL wins and that live line was just a provisional moment the late goal erased.\n")
	b.WriteString("3. Tell the STORY of the game(s): condense into 3-6 short sentences covering the actual final result, who actually nailed a scoreline (per the final points), who whiffed, any upset versus what the pool mostly predicted, and the key moments from the live commentary (\"live_story\") if present.\n")
