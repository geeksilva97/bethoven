...
func TestBettorBetsOnlyOpenUnbet(t *testing.T) {
	...
	NewBettor(deps, fp, mon, 7, time.Hour, "", 0).bet(context.Background())
...
func TestBettorMaxPerRun(t *testing.T) {
	...
	NewBettor(deps, fp, mon, 7, time.Hour, "", 2).bet(context.Background())
...
func TestBettorLockedRace(t *testing.T) {
	...
	mon := NewMonitor("test", time.Hour)
	NewBettor(deps, &fakePredictor{}, mon, 7, time.Hour, "", 0).bet(context.Background())
