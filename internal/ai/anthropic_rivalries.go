...
func rivalryPrompt(history []RoundStanding, derivedNotes string, existing []Rivalry, cfg CommentConfig) string {
	...
	if defs := defectorNames(cfg.Participation); len(defs) > 0 {
		b.WriteString("DEFECTORS — players who were actively playing and then abandoned the pool as the finish line approached: ")
		b.WriteString(strings.Join(defs, ", "))
		b.WriteString(". Fair game for a \"loyalist vs deserter\" rivalry against a player who's stuck it out — grounded in this flag, never invented.\n\n")
	}
