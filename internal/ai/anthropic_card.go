...
func cardPrompt(data CardDigestData, cfg CommentConfig) string {
	...
	b.WriteString("7. RESPECT WHEN THEY JOINED AND WHETHER THEY QUIT. If joined_late is true they registered on registered_at, after matches_before_joining games were already done — those games were never theirs to play, so don't pin them on the player or call them a slow start. If defector is true they didn't just go a bit quiet — they ABANDONED the pool down the stretch (their last recent_skips games left blank after last_pick, as the tournament reached its business end): roast that desertion SPECIFICALLY and HARD, regardless of this card's tone — they bailed before the finish, tapped out when it counted. Stay in bounds (their pool conduct only, never their identity/looks/life), never invent a reason they left, and remember a blank is still NOT a wrong pick — you're roasting the walk-away, not calling their no-shows bad predictions. A late joiner who then bailed still counts as a defector; never_picked is sitting out, not quitting — never brand a never-starter a quitter. If defector is false but recent_skips is non-zero they simply tailed off late — note it honestly (\"went quiet down the stretch\"), softer than a desertion. If middle_skips is high they were IN AND OUT — gaps in the middle of their run, not a clean campaign — which quietly costs them points; you may note the patchy attendance honestly, but those gaps are blanks, not wrong picks.\n")
